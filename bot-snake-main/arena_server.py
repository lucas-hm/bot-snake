...
def choose_enemy_direction(board, current_direction="LEFT"):
...
def is_collision(head, body, opponent_body, grow):
    if not in_bounds(tuple(head)):
        return True
    # Excluir la cabeza del propio cuerpo al verificar colisiones
    occupied = snake_cells(body[1:]) | snake_cells(opponent_body)
...
class ArenaHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        GAME_STATE["enemy_direction"] = choose_enemy_direction(
            GAME_STATE["board"], GAME_STATE.get("enemy_direction", "LEFT")
        )
